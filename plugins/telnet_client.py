...
class Login(Plugin) :
    ...
    async def main(self) :
        result = await self.rpc("login.handshake", username = "tanina.aitkhelifa")
...
class Shifumi(Plugin) :         
    def __init__ (self, client) :
        ...
        self.q = (self.p) - 1
        '''
            DECOMMENTER POUR FAIRE LA VERSION HARD
        '''
        # self.q = 0xfb
        # self.p = 0x1f7
        self.g = 2
        ...
        self.c_bot = 0
        self.m_bot = ""
        self.p_bot = 
        
        self.my_turn = True
        self.cheat = [
                legendre_symbol((int.from_bytes("PIERRE".encode(), "big") % self.p), self.p), 
                legendre_symbol((int.from_bytes("FEUILLE".encode(), "big") % self.p), self.p),
                legendre_symbol((int.from_bytes("CISEAUX".encode(), "big") % self.p), self.p)
        ]
    ...
    def process(self, data) :
        ...
        if data_b.startswith("<<< PKEY") :
            ...
            self.x_bot_parity = legendre_symbol(self.h_bot, self.p)     # On prépare la triche
            
            '''
            DECOMMENTER POUR FAIRE LA VERSION HARD
            '''
            # self.send("GROUP " + hex(self.p)[2:] + " 2 " + hex(self.q)[2:])        # On envoie notre clé
        ...
        if data_b.startswith("<<< COMMIT") :
            ...
            # On recupère la parité de l - xk
            parite_xk = self.x_bot_parity * legendre_symbol(self.r_bot, self.p)
            m_b = []
            if (legendre_symbol(self.c_bot, self.p) == 1) and (parite_xk == 1) :
                # Alors nécessairement, legendre_symbole(m, p) == 1
                for i_cheat in range(3) :
                    if self.cheat[i_cheat] == 1 :
                        m_b += [i_cheat]
            else :
                for i_cheat in range(3) :
                    if self.cheat[i_cheat] != 1 :
        ...
        if data_b.startswith("<<< OPEN") :
            ...
            if (self.r_bot != pow(self.g, k_bot, self.p_bot)) or (self.c_bot != (((int.from_bytes((self.m_bot).encode(), "big") % self.p_bot) * pow(self.h_bot, k_bot, self.p_bot)) % self.p_bot)) :
                self.send("REFEREE")
            else :
                self.send("OK")
